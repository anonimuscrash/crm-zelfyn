 ...
 *
 * O projeto tem DUAS convenções de erro:
 *   `{ error: "texto" }`                    rotas do comercial
 *   `{ error: { code, message } }`          API v1 herdada
 *
 * Tratar só uma imprime "[object Object]" na tela — o pior tipo de
 * erro, porque some justamente com a informação de que o operador
 * precisa para entender o que houve.
 *
 * Centralizado aqui para o problema não voltar em cada tela nova.
 */
export async function errorMessageFrom(
  ...
  if (corpo?.error && typeof corpo.error === 'object' && corpo.error.message) {
