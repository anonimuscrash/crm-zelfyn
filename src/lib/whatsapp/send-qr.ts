...
export async function resolveQrConnection(
  ...
  // Permissão de resposta, verificada no BANCO.
  //
  // A policy de `messages` já barraria a gravação, mas o erro que
  // chegaria seria de RLS — genérico, sem dizer que o problema é
  // acesso à conexão. Recusar aqui permite a frase certa.
  // Só recusa quando a resposta é um `false` EXPLÍCITO.
  //
  // Se a RPC falhar — indisponível, erro de rede, cliente sem
  // suporte a `rpc` — seguimos adiante. A barreira real é a policy de
  // `messages`, que não depende desta chamada; esta função existe
  // apenas para dar uma mensagem melhor que o erro de RLS.
  //
  // Tratar a falha como negação transformaria uma indisponibilidade
  // momentânea em "você não tem permissão", que é errado e assusta.
  let podeResponder: boolean | null = null;
  try {
    const { data } = await db.rpc('can_reply_to_conversation', {
      p_conversation_id: conversationId,
    });
    if (typeof data === 'boolean') podeResponder = data;
  } catch {
    podeResponder = null;
  }
  ...
  if (podeResponder === false) {
    throw new QrSendError(
      'Você não atende neste número de WhatsApp. Peça ao titular da conta para incluir você.',
      403
    );
  }
