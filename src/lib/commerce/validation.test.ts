...
describe('parseCreateOrderInput', () => {
  ...
  // A mensagem é em português porque chega ao operador na tela.
  // "items[0] needs a product_id or a product_name" é útil para quem
  // escreveu a API e inútil para quem está vendendo.
  it('requires each line to identify what was sold', () => {
    expect(() => parseCreateOrderInput({ items: [{ quantity: 1 }] })).toThrow(
      /precisa de um produto ou de um nome/
    );
  });
