...
import { ConnectionAgents } from '@/components/settings/connection-agents';
import {
  EmptyState,
  ErrorState,
  formatDateTimeBR,
...
export function WhatsAppConnectionSettings() {
  ...
  const limiteAtingido = Boolean(uso && uso.used >= uso.allowed);
  ...
  const carregar = useCallback(async () => {
    try {
      ...
      const json = (await res.json()) as {
        connections: Conexao[];
        qrAvailable: boolean;
        usage?: { used: number; allowed: number; plan: string };
      };
      setConexoes(json.connections);
      setQrAvailable(json.qrAvailable);
      setUso(json.usage ?? null);
      setErro(null);
    } catch (e) {
    ...
    <div className="space-y-5">
      ...
      {podeGerenciar ? (
        <section className="rounded-lg border border-border bg-card p-4">
          ...
          {!qrAvailable ? (
            ...
            <div className="mt-3 flex flex-wrap items-end gap-2">
              ...
              <Button
                ...
                className="h-9"
                onClick={criar}
                disabled={criando || !nome.trim() || limiteAtingido}
              >
          ...
          {/* Uso e teto.
              Mostrado ANTES de o operador tentar: descobrir o limite
              ao clicar e receber erro é pior que saber de antemão. */}
          {uso ? (
            <p
              className={cn(
                'mt-2 text-[11px]',
            ...
            >
              {t('connectionUsage', { used: uso.used, allowed: uso.allowed })}
              {limiteAtingido ? ` · ${t('limitReached')}` : ''}
            </p>
          ) : null}
    ...
    <li className="rounded-lg border border-border bg-card">
      ...
      {expanded ? (
        <div className="border-t border-border px-4 py-4">
          ...
          {/* Quem atende. Só quando conectada: escolher atendentes de
              uma conexão que ainda não pareou é decidir sobre algo
