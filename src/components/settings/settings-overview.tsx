...
export function SettingsOverview({
  ...
  // Estado das integrações comerciais.
  //
  // Consultadas juntas e com falha silenciosa: um cartão que não
  // carrega mostra o texto neutro em vez de sumir. A visão geral
  // existe para dar um panorama — um buraco nela é pior que um
  // cartão sem número.
  const [integracoes, setIntegracoes] = useState<{
    frete: { ativo: boolean; configurado: boolean } | null;
    pagamento: { ativo: boolean; chaves: number; auto: boolean } | null;
    whatsappQr: { existe: boolean; conectada: boolean; numero: string | null } | null;
  }>({ frete: null, pagamento: null, whatsappQr: null });
  ...
  useEffect(() => {
    ...
    void (async () => {
      const [frete, pagamento, whats] = await Promise.all([
      ...
      setIntegracoes({
        frete: frete
          ? { ativo: Boolean(frete.is_enabled), configurado: Boolean(frete.is_configured) }
          : null,
        pagamento: pagamento
          ? {
              ativo: Boolean(pagamento.dotfy_enabled && pagamento.dotfy_configured),
              chaves: Number(pagamento.pix_key_count ?? 0),
              auto: Boolean(pagamento.webhook_enabled),
            }
          : null,
        whatsappQr: (() => {
          const lista = (whats?.connections ?? []) as {
            status: string;
            phone_number: string | null;
          }[];
          if (lista.length === 0) return null;
          const ativa = lista.find((c) => c.status === 'connected');
          return {
            existe: true,
            conectada: Boolean(ativa),
            numero: ativa?.phone_number ?? null,
          };
        })(),
      });
  ...
  const tiles: {
    ...
    {
      section: 'whatsapp',
      loading: whatsappLoading || integracoesLoading,
      // O cartão considera OS DOIS métodos.
      //
      // Antes olhava só `whatsapp_config`, a configuração da Meta.
      // Quem usa exclusivamente QR via "Ainda não configurado" com a
      // sessão conectada e mensagens chegando — o painel dizia o
      // oposto do que a Inbox mostrava.
      subtitle: integracoes.whatsappQr?.conectada ? (
        <>
          <StatusDot tone="ok" /> {t('connected')}
          {integracoes.whatsappQr.numero
            ? ` · +${integracoes.whatsappQr.numero}`
            : ''}
        </>
      ) : integracoes.whatsappQr?.existe ? (
        <>
    ...
    {
      section: 'payments',
      loading: integracoesLoading,
      subtitle: (() => {
        const p = integracoes.pagamento;
        if (!p) return t('managePayments');

        const partes: string[] = [];
        if (p.ativo) {
          partes.push(
            p.auto ? t('dotfyAuto') : t('dotfyManual')
          );
        }
        if (p.chaves > 0) partes.push(t('pixKeysCount', { count: p.chaves }));

        // Nenhum dos dois configurado: o cartão diz o que FALTA, não
        // "0 chaves" — zero é um número, não uma orientação.
        if (partes.length === 0) return t('notSetup');

        return (
          <>
            <StatusDot tone="ok" /> {partes.join(' · ')}
          </>
        );
      })(),
    },
    {
      ...
      loading: integracoesLoading,
      subtitle: (() => {
        const f = integracoes.frete;
        if (!f) return t('manageShipping');
        if (!f.configurado) return t('notSetup');
        return f.ativo ? (
          <>
            <StatusDot tone="ok" /> {t('shippingActive')}
          </>
        ) : (
          <>
            <StatusDot tone="muted" /> {t('shippingInactive')}
          </>
        );
      })(),
    },
