...
/**
 * Papel de parede da área de conversa (thread ativa e estado vazio).
 *
 * A arte em si — e a troca claro/escuro — vive em `.chat-wallpaper`,
 * no `globals.css`. Aqui fica só a classe, porque a seleção do modo
 * precisa acontecer em CSS: o script de boot do `layout.tsx` aplica
 * o `data-mode` antes da hidratação, e escolher a imagem em React
 * mostraria o fundo do tema errado por um frame.
 *
 * Definido uma vez no escopo do módulo para que os dois caminhos de
 * render não divirjam.
 */
const CHAT_WALLPAPER_CLASS = "chat-wallpaper";
...
export function MessageThread({
  ...
  // Conexão QR: derivada da própria conversa, sem prop nova
  // atravessando componentes intermediários.
  const conexaoQr = Boolean(
    (conversation as { whatsapp_connection_id?: string | null } | null)
   ...
   * Janela de 24 horas.
   *
   * É REGRA DA META CLOUD API, não do WhatsApp. A API oficial só
   * permite mensagem livre nas 24h após o cliente escrever; depois,
   * exige template aprovado.
   *
   * Uma sessão QR não tem essa restrição — é o WhatsApp Web, e você
   * responde quando quiser. Mostrar o contador ali anuncia um limite
   * que não existe, e pior: BLOQUEIA o envio ao zerar, impedindo uma
   * resposta que funcionaria.
   *
   * `conexaoQr` desliga o contador e o bloqueio de uma vez.
   */
  const sessionInfo = useMemo(() => {
    if (conexaoQr) return { expired: false, remaining: "" };
  ...
  if (!conversation || !contact) {
    return (
      <div className={cn("flex flex-1 flex-col items-center justify-center", CHAT_WALLPAPER_CLASS)}>
  ...
  return (
    ...
    // Issue #257.
    <div className={cn("flex min-w-0 flex-1 flex-col", CHAT_WALLPAPER_CLASS)}>
