...
export function NewSaleDrawer({
  ...
  /** Nome para exibir enquanto o contato pré-selecionado não carrega. */
  ...
  const reset = useCallback(() => {
    setLines([]);
    // O contato pré-selecionado é reposto pelo efeito acima na
    // próxima abertura; limpar aqui mantém o formulário previsível.
    setContact(null);
    setStatus('new');
  ...
  // Cliente pré-selecionado (abertura a partir de uma conversa).
  //
  // Antes o id era enviado no payload mas o campo aparecia VAZIO — o
  // operador via "sem cliente vinculado" e, achando que faltava
  // preencher, buscava e selecionava o mesmo contato de novo. A venda
  // saía certa, mas a tela mentia sobre o que ia acontecer.
  useEffect(() => {
    if (!open || !presetContactId) return;
    if (contact?.id === presetContactId) return;

    let cancelado = false;

    fetch(`/api/v1/contacts/${encodeURIComponent(presetContactId)}`)
      .then((r) => (r.ok ? r.json() : null))
      .then((json: { data?: ContactOption } | null) => {
        if (cancelado) return;
        if (json?.data) {
          setContact(json.data);
        } else if (presetContactName) {
          // Fallback: mostra o que sabemos em vez de deixar em
          // branco. O envio usa o id de qualquer forma.
          setContact({
            id: presetContactId,
            name: presetContactName,
            phone: '',
          });
        }
      })
      .catch(() => {
        if (!cancelado && presetContactName) {
          setContact({ id: presetContactId, name: presetContactName, phone: '' });
        }
      });
  ...
  }, [open, presetContactId, presetContactName, contact?.id]);
  ...
  // ---- live totals ----
  /**
   * Item avulso sem nome não pode ser enviado.
   *
   * O backend recusa com "items[0] needs a product_id or a
   * product_name" — mensagem com índice de array, inútil para quem
   * está vendendo. Barrar aqui é o lugar certo: o operador vê qual
   * campo falta enquanto ainda está olhando para ele.
   */
  const itemSemNome = lines.some(
    (l) => l.productId === null && !l.productName.trim()
  );
  ...
  /**
   * Item avulso — venda de algo que não está no catálogo.
   *
   * O backend já aceitava: `order_items.product_id` é nullable e
   * `product_name` guarda o nome como snapshot. Faltava só a
   * interface.
   *
   * Preço e custo entram zerados e o operador digita. Sem produto,
   * não há de onde herdar — e chutar um valor seria pior que pedir.
   *
   * Nasce com o nome vazio de propósito: o campo fica em foco e o
   * operador escreve o que vendeu. Um "Item avulso" pré-preenchido
   * acabaria salvo assim em metade das vendas.
   */
      ...
      >
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        {/* Item avulso tem o nome editável; item de
                            catálogo, não. Deixar o nome do produto
                            cadastrado editável aqui criaria vendas
                            com o mesmo produto sob nomes diferentes,
                            e o ranking pararia de somar. */}
                        {line.productId === null ? (
                          <Input
                            value={line.productName}
                            placeholder={t('freeItemPlaceholder')}
                            autoFocus
                            maxLength={120}
                            onChange={(e) =>
                              patchLine(line.key, {
        ...
        <footer className="shrink-0 border-t border-border bg-card px-4 py-3">
          <div className="mb-3 space-y-1 text-[13px]">
            ...
            {/* CUSTOS DIRETOS, não "Total".
                A linha soma frete, custo dos produtos e taxa de
                pagamento — tudo que sai do bolso. Rotulá-la como
                "Total" fazia o operador ler R$ 16,00 negativos como
                o valor da venda, num resumo em que a leitura errada
                leva a decisão errada sobre desconto e preço. */}
            ...
            {/* Botão desabilitado sem explicação é frustrante: o
                operador clica, nada acontece, e ele não sabe por quê. */}
            {itemSemNome ? (
          ...
          <div className="flex gap-2">
            ...
            <Button
              className="flex-1"
              onClick={submit}
              disabled={saving || lines.length === 0 || itemSemNome}
            >
              {saving ? (
