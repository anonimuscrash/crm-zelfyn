 ...
 *
 * O `.map()` que atualiza o preview troca os campos NO LUGAR — ele
 * devolve um array na mesma ordem de antes. Era por isso que enviar
 * uma mensagem mudava o texto da prévia mas não movia a conversa
 * para o topo: o dado estava certo, a posição é que não era
 * recalculada. Só um refresh da página, que relia a lista já
 * ordenada do banco, colocava tudo no lugar.
 *
 * Conversa sem `last_message_at` vai para o fim em vez de virar
 * `NaN` na subtração e embaralhar a lista inteira.
 */
function sortByRecency<T extends { last_message_at?: string | null }>(
  lista: T[],
): T[] {
...
function InboxPageInner() {
  ...
  useEffect(() => {
    const checkConnection = async () => {
      ...
      // A conta está conectada se QUALQUER método estiver de pé: a
      // API oficial da Meta ou uma sessão QR. Checar só
      // `whatsapp_config` fazia o aviso "WhatsApp não conectado"
      // aparecer para quem usa exclusivamente QR — com a Inbox
      // recebendo mensagens normalmente logo abaixo do aviso.
      const [meta, qr] = await Promise.all([
        supabase
      ...
      const metaOk = meta.data?.status === "connected";
      const qrOk = (qr.data?.length ?? 0) > 0;

      setWhatsappConnected(metaOk || qrOk);
    };
    ...
    (event: {
      ...
      if (event.eventType === "UPDATE") {
        if (knownConvIdsRef.current.has(conv.id)) {
          ...
          // Reordena também aqui: este UPDATE é o que chega quando o
          // banco grava o novo `last_message_at`, inclusive nos envios
          // feitos pelo próprio operador.
          setConversations((prev) =>
            sortByRecency(
              prev.map((c) =>
                c.id === conv.id
  ...
  return (
    // `h-full` em vez de `100vh - 3.5rem`: a altura do header é uma
    // suposição que quebra se ele mudar, e no mobile a barra de
    // endereço do navegador altera o valor de `vh` ao rolar. O pai já
    // tem altura definida — herdar dele é correto por construção.
