...
//   ├──────────────────────┼────────────┼────────────────────────────┤
//   │ loading              │ —          │ spinner                    │
//   │ ok:false (any reason)│ —          │ erro amigável + cadastro   │
//   │ ok:true              │ signed out │ "Criar conta e entrar"     │
//   │ ok:true              │ signed in  │ "Aceitar convite" → redeem │
//   └──────────────────────┴────────────┴────────────────────────────┘
//
...
const FAIL_COPY: Record<PeekFail['reason'], { title: string; body: string }> = {
  not_found: {
    ...
    body: 'Este link não corresponde a um convite válido. Confira o endereço ou peça um novo para quem te convidou.',
  },
  used: {
  ...
  expired: {
    ...
    body: 'Este convite expirou. Peça ao administrador da conta para gerar outro — leva alguns segundos.',
  },
  server_error: {
    title: 'Algo deu errado',
    body: 'Não conseguimos verificar este convite agora. Atualize a página daqui a pouco.',
  },
};
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(
            payload.error ||
              'Você já pertence a outra conta. Entre com outro e-mail para aceitar este convite.',
          );
        } else {
        ...
        <Card className="w-full max-w-md border-border bg-card">
          ...
          <CardContent className="flex flex-col gap-3">
            ...
            <p className="text-center text-xs text-muted-foreground">
              ...
              <span className="text-muted-foreground">{peek.account_name}</span>. A
          ...
          <DialogContent className="bg-popover border-border sm:max-w-md">
            ...
            <div className="space-y-2 py-2 text-xs text-muted-foreground">
              <p>
                ...
                O link do convite continua válido enquanto não
                expirar.
              </p>
            </div>
