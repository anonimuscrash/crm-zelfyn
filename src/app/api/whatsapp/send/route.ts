...
export async function POST(request: Request) {
  try {
    ...
    //
    // Se a conversa pertence a uma conexão QR, o envio vai pelo
    // adapter dessa sessão. Caso contrário segue exatamente como
    // antes, pelo caminho da Meta Cloud API — quem usa a API oficial
    // não percebe diferença nenhuma.
    //
    // A checagem vem ANTES do core da Meta porque é ele que consulta
    // `whatsapp_config` e devolve "WhatsApp not configured": uma
    // conta que só usa QR não tem essa linha e receberia o erro
    // errado.
    try {
      const qr = await resolveQrConnection(supabase, accountId, conversationId)

      if (qr) {
        const resultado = await sendViaQr(
          supabase,
          accountId,
          userId,
          qr,
        ...
        return NextResponse.json({
          ...
          whatsapp_message_id: resultado.externalId,
        })
      }
    } catch (err) {
      if (err instanceof QrSendError) {
        return NextResponse.json({ error: err.message }, { status: err.status })
      }
      throw err
    }
