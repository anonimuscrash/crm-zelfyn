...
function getBaseUrl(request: Request): string {
  // O HOST DA REQUISIÇÃO VENCE quando é um domínio conhecido.
  //
  // `NEXT_PUBLIC_SITE_URL` tinha prioridade absoluta, e o resultado
  // era um convite gerado a partir de operza.com.br apontando para
  // o domínio antigo — porque a variável ficou desatualizada depois
  // de uma troca de domínio.
  //
  // O host de onde o convite foi criado é a informação mais
  // confiável: é onde o operador está, e onde o convidado vai
  // conseguir entrar. A variável continua servindo de fallback para
  // quando não há host (job em background, por exemplo).
  const allowListInicial = parseAllowedHosts();
  const hostReq = request.headers
  ...
  if (hostReq && isHostAllowed(hostReq, allowListInicial)) {
    return `${protoReq}://${hostReq}`.replace(/\/+$/, "");
  }
